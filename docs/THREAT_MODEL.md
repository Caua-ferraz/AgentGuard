...
- **Secrets in requests are masked before they are stored or shown** (`serve --audit-redact`, default on since v1.2.0): the audit trail, `GET /v1/audit`, the SSE stream, the dashboard and the pending list see `[REDACTED]` for keys, tokens and passwords an agent puts in a command. The approval store keeps the original request for replay matching. See [OPERATIONS § Audit redaction](OPERATIONS.md#audit-redaction).
