...
- **Glob matching is string matching.** An agent can express the same action differently (`find / -delete` for `rm -rf`). Policies are a high-signal filter, not a semantic authorisation model — pair with OS isolation (README).
