...
If you build the TypeScript SDK from source (`plugins/typescript`), the floor moves to Node ≥ 20 and the build now requires TypeScript 6. Code that consumes the compiled package is unaffected — the runtime behaviour and API surface are identical. *(Corrected in v1.1.1: the TypeScript SDK was not published to npm at this release, and `@agentguard/sdk` is an unrelated package. The SDK is on npm as `@lictorate/agentguard` from 1.1.1.)*
...
- **Docker:** no prebuilt image is published — build it from the repository's `Dockerfile` (`docker build -t agentguard:0.5.2 .`; see [`SETUP.md`](../SETUP.md#using-docker)). *(Corrected in v1.1.1: this line previously said to `docker pull` a published image; none has ever been published.)*
- **Python SDK:** `pip install --upgrade agentguardproxy`.
- **TypeScript SDK:** not published to npm at this release — build it from `plugins/typescript`. *(Corrected in v1.1.1: the TypeScript SDK was not published to npm at this release, and `@agentguard/sdk` is an unrelated package. The SDK is on npm as `@lictorate/agentguard` from 1.1.1.)* If you were locked at `^5.3.0` for TypeScript in your project for any reason, the SDK now builds against TS 6 — but for *consumers* of the compiled package (the normal case), no action.
