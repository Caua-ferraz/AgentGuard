...
//     satisfy another's condition (audit B27).
...
//     over it and throw it away (audit B1).
...
// deployment is exposed to audit B27 and needs to wire a PriorActionQuerier.
func warnLegacyHistoryPath() {
	legacyHistoryWarnOnce.Do(func() {
		log.Printf("WARNING: require_prior is being evaluated through the legacy HistoryQuerier, " +
			...
			"(audit B27), and the lookup reads the audit log synchronously on the /v1/check path (audit B1). " +
